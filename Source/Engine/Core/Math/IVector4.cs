...
internal interface IVector4<TSelf, TComponent> : IVector3<TSelf, TComponent>
    where TSelf : unmanaged, IVector4<TSelf, TComponent>
    where TComponent : IFloatingPointIeee754<TComponent>
{
    /// <summary>
    /// Returns a vector containing the 4D Cartesian coordinates of a point specified in Barycentric coordinates relative to a 4D triangle.
    /// </summary>
    ...
    static virtual void Barycentric(in TSelf value1, in TSelf value2, in TSelf value3, TComponent amount1, TComponent amount2, out TSelf result)
    {
        result = TSelf.Barycentric(in value1, in value2, in value3, amount1, amount2);
    }

    /// <inheritdoc cref="Barycentric(in TSelf, in TSelf, in TSelf, TComponent, TComponent, out TSelf)"/>
    ...
    static virtual void Transform(in TSelf vector, in Quaternion rotation, out TSelf result)
    {
        result = TSelf.Transform(in vector, in rotation);
    }

    /// <inheritdoc cref="Transform(in TSelf, in Quaternion, out TSelf)"/>
    ...
    /// <param name="result">When the method completes, contains the transformed vector.</param>
    static virtual void Transform(in TSelf vector, in Matrix transform, out TSelf result)
    {
        result = TSelf.Transform(in vector, in transform);
    }

    /// <inheritdoc cref="Transform(in TSelf, in Matrix, out TSelf)"/>
    /// <returns>The transformed vector.</returns>
